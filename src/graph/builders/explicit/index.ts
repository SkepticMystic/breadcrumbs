...
const typed_link: GraphBuilder = (
	graph,
	plugin,
	[obsidian_files, dataview_files],
) => {
	if (obsidian_files) {
		// First add all nodes
		obsidian_files.forEach((source_file) => {
		...
		obsidian_files.forEach((source_file) => {
			const source_cache =
				plugin.app.metadataCache.getFileCache(source_file);
		...
		dataview_files.forEach((page) => {
			graph.addNode(page.file.path, {
				resolved: true,
				aliases: page.aliases ?? undefined,
			});
		});
